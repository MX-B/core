...
public enum TestUtils {
	...
	/**
	 * Checks if a given {@link Method method} have the specified annotations
	 */
	@SafeVarargs
	public static void testAnnotations(final Method method, final Class<? extends Annotation>... annotations) {
		Stream.of(annotations).filter(Objects::nonNull).forEach(annotation -> {
			assertThat(method.getAnnotation(annotation), is(notNullValue()));
		});
	}
	...
	public static void testLogger(final Class<?> klass)
		...
		assertThat(logFieldClass, is(equalTo(Logger.class)));
